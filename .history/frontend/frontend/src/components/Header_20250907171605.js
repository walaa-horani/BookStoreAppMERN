    ...
    const renderUserActions=()=>{
        ...
        <span className={`text-sm ${isScrolled ? "text-gray-700" : "text-gray-700"}`}>
          ...
          {isAdmin ? (
           ...
           <ShoppingBasket/>
            {count > 0 && (
          <span className="absolute -top-2 -right-2 min-w-5 h-5 px-1 rounded-full bg-[#F86D72] text-white text-xs flex items-center justify-center">
            {count}
          </span>
        )}
           <Heart/> 
