...
function AddBook() {
  ...
  useEffect(() => {
    // انتظر لحد ما يخلص تحميل الأوث
    if (loading) return

    // لو مو أدمن → رجّع على الهوم
    if (!isAuthenticated || !isAdmin) {
      navigate('/', { replace: true })
      return
    }

    // جلب التصنيفات بعد التأكد من الصلاحية
    const loadCats = async () => {
      try {
        const res = await fetch('http://localhost:5000/category/getCategories', {
          method: 'GET',
          credentials: 'include', // لو مسارك محمي/يحتاج كوكي
        })
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:5000/admin/createBook', {
        method: 'POST',
        credentials: 'include',
        body: fd, // لا تضيف Content-Type مع FormData
      })
  ...
  // أثناء تحميل حالة الأوث ما نعرض شيء (أو رجّع سكيليتون)
  if (loading) return null
  // لو صار redirect، هذا يمنع فلاش موقت
