...
const CREATE_URL = "http://localhost:5000/books/createBook";
const CATEGORIES_URL = "http://localhost:5000/category/getCategories"; 
// إذا مسارك مختلف (مثلاً GET /category) عدّل الرابط
 ...
 function AddBook() {
  ...
  const [form, setForm] = useState({
    ...
    price: "",
    stock: "",
    category: "",         // لازم يكون _id من الدروبداون
    discountPercent: "",
    isFeautred: false,    // مطابق للاسم في الباك
    isOnSale: false,
    coverImage: null,     // ملف الصورة
  });
  ...
  // جلب التصنيفات لعرضها بالدروبداون
  useEffect(() => {
    const loadCats = async () => {
      try {
        const res = await fetch(CATEGORIES_URL);
        const data = await res.json();
        // توقّعنا أن الـ API يرجّع مصفوفة تصنيفات [{_id, name}, ...]
  ...
  const onSubmit = async (e) => {
    ...
    if (form.discountPercent) fd.append("discountPercent", String(form.discountPercent));
    // booleans (Mongoose بيحوّل "true"/"false" تلقائيًا)
    fd.append("isFeautred", String(form.isFeautred));
    fd.append("isOnSale", String(form.isOnSale));
    if (form.coverImage) fd.append("coverImage", form.coverImage); // لازم نفس الاسم بـ multer.single('coverImage')
