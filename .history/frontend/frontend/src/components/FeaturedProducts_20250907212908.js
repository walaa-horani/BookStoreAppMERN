...
function FeaturedProducts() {
    ...
    const { books, addToCart,cartLoading, getBookStock, isInCart, getCartQuantity } = useCart();
  ...
  return (
    ...
    <div className='flex items-center justify-center flex-col gap-5   border p-4 border-gray-300  rounded-lg'>
      ...
      <button  disabled={cartLoading}  onClick={()=> addToCart(book._id)} className="whitespace-nowrap w-44">{cartLoading ? "Adding To cart" : book.stock=== 0 ? "Out Of stock":"Add To cart"}</button>
        </div>
         )
         })}
