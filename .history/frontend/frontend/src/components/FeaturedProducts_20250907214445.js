...
function FeaturedProducts() {
  ...
  const { addToCart, getBookStock, isInCart, getCartQuantity } = useCart();

  // ⬇️ هون بدال boolean واحد
  ...
  // ⬇️ دالة وسيطة تضبط حالة الزر حسب bookId
    ...
    <div className='mt-10'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 '>
        {featuredBooks?.map((book) => {
          ...
          const isLoading = loadingIds.includes(book._id); // ⬅️ تحقق لكل زر
          ...
          return (
            <div key={book._id} className='flex items-center justify-center flex-col gap-5 border p-4 border-gray-300 rounded-lg'>
              ...
              {inCart && <div>{cartQuantity} in cart</div>}
