...
router.post("/add", async(req,res)=>{
    ...
    try {
        ...
        const fakeUserId = "65c6dbe37e73a22a6b7d1a24";
        console.log("Received bookId:", bookId); 

        const book = await Book.findById(bookId)


         let cart = await Cart.findOne({user:fakeUserId }).populate("items.book","title price coverImage stock")

        if(!cart){
            cart = new Cart({user:fakeUserId ,items:[]})
